...
namespace HowellMatrix
{
		...
		: num_cols_(that.num_cols_)
	{
		...
		while (beg != end) {
			SparseTArray<T>* newCopy = new SparseTArray<T>(**beg);
	...
	template <typename T>
	SparseRowMatrix<T>::SparseRowMatrix(SparseRowMatrix*& that)
		: num_cols_(that->num_cols_)
	{
		typename RowVector::const_iterator beg, end;
		beg = that->mat_.begin();
	...
	SparseRowMatrix<T> SparseRowMatrix<T>::operator=(const Matrix<T>& that_gen)
	{
		...
		while (beg != end) {
			SparseTArray<T>* newCopy = SparseTArray<T>::copy(*beg);
			mat_.push_back(newCopy);
			beg++;
		}
		return *this;
	...
	inline bool SparseRowMatrix<T>::operator==(const Matrix<T> & that_gen) const {
		...
		for (it_this = mat_.begin(), it_that = that->mat_.begin();
			...
			it_this++, it_that++)
		{
			SparseTArray<T>* this_row (*it_this);
			SparseTArray<T>* that_row (*it_that);
	...
	inline size_t SparseRowMatrix<T>::GetLeadingIndex(size_t r) const {
		auto row = mat_[r];
		return row->GetLeadingIndex();
	}

	template <typename T>
	inline size_t SparseRowMatrix<T>::GetLeadingRank(size_t r) const {
		// std::cout << "GetLeadingRank: " << r << " " << mat_.size() << std::endl;
		auto row = mat_[r];
		// row->Print(std::cout);
		return row->GetLeadingRank();
	}

	template <typename T>
	inline size_t SparseRowMatrix<T>::GetSize() const {
		return mat_.size();
	}

	template <typename T>
	inline const TArray<T>* SparseRowMatrix<T>::GetRow(size_t r) const {
		auto row = mat_[r];
		return row->GetTArray();
	}

	// template <typename T>
	// inline const SparseTArray<T>* SparseRowMatrix<T>::GetConstSparseRow(size_t r) const {
	// 	SparseTArray<T>* row = mat_[r];
	// 	return std::make_unique<SparseTArray<T> >(row);
	// }

	template <typename T>
	inline SparseTArray<T>* SparseRowMatrix<T>::GetSparseRow(size_t r) {
		auto row = mat_[r];
		return row;
	}

	template <typename T>
	inline SparseTArray<T>* SparseRowMatrix<T>::operator[](size_t r) {
		return mat_[r];
	}
	...
	void SparseRowMatrix<T>::addRow(const T* row) {
		SparseTArray<T>* r = new SparseTArray<T>(row, NumCols());
		mat_.push_back(r);
	}

	template <typename T>
	void SparseRowMatrix<T>::addRow(const std::vector<T> & row) {
		SparseTArray<T>* r = new SparseTArray<T>(row);
		mat_.push_back(r);
	}
	...
	template <typename T>
	void SparseRowMatrix<T>::addRow(const SparseTArray<T>* & row) {
		// std::cout << "addRow: " << row->Size() << std::endl;
		mat_.push_back(row);
		// std::cout << "addRow add: " << mat_[mat_.size()-1]->Size() << std::endl;
	}

	template <typename T>
	void SparseRowMatrix<T>::addRow(size_t index) {
		SparseTArray<T>* r = new SparseTArray<T>(*(mat_[index]));
		mat_.push_back(r);
	}
	...
	void SparseRowMatrix<T>::insertVals(size_t begin, size_t len,
		SparseRowMatrix<T>* & vals)
	{
		typename RowVector::iterator it = mat_.begin();
		for (; it != mat_.end(); it++)
		{
			(*it)->insertVals(begin, len, *(vals->GetSparseRow(it - mat_.begin())));
		}
		num_cols_ += len;
	}
	...
	SparseRowMatrix<T> SparseRowMatrix<T>::parse(std::istream & buf) {
		...
		std::vector<SparseTArray<T>* > rows;
		...
		for (size_t row = 0; row < num_rows && buf.good(); row++) {
			SparseTArray<T>* this_row = new SparseTArray<T>(num_cols);
			size_t col;
	...
	{
		auto row = mat_[out_index];
		SparseTArray<T>::vectorMul(row, mult);
	}
	...
	{
		auto row = mat_[out_index];
		SparseTArray<T>::vectorLinearCombination(row, *mat_[in_index_1], mult_1, *mat_[in_index_2], mult_2);
	}
}
