...
namespace CFL_OBDD
{
	namespace HowellMatrix
	{
		...
		template <typename T>
		TArray<T>::TArray(const T* ia, size_t length)
		...
		template <typename T>
		ref_ptr<TArray<T> > TArray<T>::projectOut(const std::vector<size_t> & projectOutIndices) const
		{
			ref_ptr<TArray<T> > ret = new TArray(len - projectOutIndices.size());
			size_t curIndex = 0;
			for (size_t i = 0; i < len; i++)
			{
		...
		*--------------------------------------------------------------*/
		...
		template <typename T>
		ref_ptr<TArray<T> > ModularRow<T>::getTArray() {
			return m;
		}
		template <typename T>
		ref_ptr<TArray<T> > ModularRow<T>::getConstTArray() const{
			ref_ptr<TArray<T> > ret = new TArray<T>(*m);
			return ret;
		}
		template <typename T>
		void ModularRow<T>::setMat(ref_ptr<TArray<T> > mat) {
			m = mat;
		}
		...
		template <typename T>
		ModularRow<T>::ModularRow(const ref_ptr<TArray<T> > & _m, size_t n) {
			m = _m;
		...
		template <typename T>
		ModularRow<T>::ModularRow(const ref_ptr<TArray<T> > & _m, size_t _li, size_t _lr) {
		...
		template <typename T>
		ref_ptr<ModularRow<T> > ModularRow<T>::copy(const ref_ptr<ModularRow<T> > a)
		{
			ref_ptr<TArray<T> > tmp = a->m;
			size_t n = tmp->getLength();

			ref_ptr<TArray<T> > ans = new TArray<T>(n);
			for (size_t i = 0; i<n; i++)
				ans->set(i, tmp->get(i));

			ref_ptr<ModularRow> ret = new ModularRow(ans, n);
			return ret;
		}
		...
		template <typename T>
		std::vector<ref_ptr<ModularRow<T> > > ModularRow<T>::copy(const std::vector<ref_ptr<ModularRow<T> > > & v)
		{
			typename std::vector<ref_ptr<ModularRow> > ans;
			typename std::vector<ref_ptr<ModularRow> >::const_iterator it;

			for (it = v.begin(); it != v.end(); it++) {
				ref_ptr<ModularRow> t = copy(*it);
				ans.push_back(t);
			}
			return ans;
		}
