...
namespace CFL_OBDD
{
	namespace HowellMatrix
	{
		...
		template <typename T>
		HowellMatrix<T>::HowellMatrix(size_t vecSize, bool keep_zero_vector)
		...
		*/
		...
		template <typename T>
		inline bool HowellMatrix<T>::isEqual(const ref_ptr<GenericHowellMatrix> & that) const {
			const HowellMatrix<T>* that_cast = dynamic_cast<const HowellMatrix<T>* >(that.get_ptr());
			return isEqual(*that_cast);
		}
		...
		template <typename T>
		inline bool HowellMatrix<T>::isEqual(const ref_ptr<HowellMatrix<T> > & that) const {
			return isEqual(*that);
		}
		...
		template <typename T>
		void HowellMatrix<T>::insertRow(const ref_ptr<TArray<T> > & f, bool perform_howellize) {
			insertRow(*f);
		}
		template <typename T>
		...
		template <typename T>
		void HowellMatrix<T>::insertRow(const ref_ptr<SparseTArray<T> > & f, bool perform_howellize) {
		...
		template <typename T>
		ref_ptr<GenericHowellMatrix> HowellMatrix<T>::generic_project_left(size_t index) const {
			return project_left(index).get_ptr();
		}

		// Project away (some) left-hand columns.
		// Really, this is shorthand for project([index,index+1,...,num_vars])
		template <typename T>
		ref_ptr<HowellMatrix<T> > HowellMatrix<T>::project_left(size_t index) const {
			UWAssert::UWAssert::shouldNeverHappen(!howellized_);
			std::vector<ref_ptr<SparseTArray<T> > > new_mat;
			for (size_t i = 0; i < size(); i++)
			{
				if (mat_->GetLeadingIndex(i) >= index)
				{
					ref_ptr<SparseTArray<T> > projected_row = new SparseTArray<T>(*(mat_->GetSparseRow(i)));
			...
			ref_ptr<HowellMatrix<T> > ret = new HowellMatrix<T>(GetVectorSize() - index);
		...
		template <typename T>
		inline void HowellMatrix<T>::concatRight(const ref_ptr<HowellMatrix<T> > that) {
			concatRight(*that);
		}
		...
		void HowellMatrix<T>::concatRight(const HowellMatrix<T> & that) {
			...
			ref_ptr<SparseTArray<T> > zero_row = new SparseTArray<T>(mat_->NumCols());
			...
			ref_ptr<SparseTArray<T> > that_zero_row = new SparseTArray<T>(that.mat_->NumCols());
		...
		template <typename T>
		inline void HowellMatrix<T>::concatDown(const ref_ptr<HowellMatrix<T> > that) {
			concatDown(*that);
		}
		template <typename T>
		inline void HowellMatrix<T>::concatDown(const HowellMatrix<T> & that) {
			const std::vector<ref_ptr<SparseTArray<T> > > rows = that.mat_->mat_;
		...
		// Insert set of rows
		template <typename T>
		void HowellMatrix<T>::insertSetOfRows(const std::vector<ref_ptr<ModularSquareMatrix<T> > > & vectorSet, bool perform_howellize) {
			for (size_t i = 0; i < vectorSet.size(); i++) {
				UWAssert::UWAssert::shouldNeverHappen(mat_->NumCols() != vectorSet[i]->getDim() * vectorSet[i]->getDim());
				ref_ptr<SparseTArray<T> > r = new SparseTArray<T>(vectorSet[i]->getConstMatrix(), mat_->NumCols());
				mat_->addRow(r);
			}
			howellized_ = false;
			if (perform_howellize)
		...
		template <typename T>
		void HowellMatrix<T>::insertSetOfRows(const std::vector<ref_ptr<ModularRow<T> > > & vectorSet, bool perform_howellize) {
			for (size_t i = 0; i < vectorSet.size(); i++) {
		...
		template <typename T>
		void HowellMatrix<T>::insertSetOfRows(const std::vector<ref_ptr<TArray<T> > > & vectorSet, bool perform_howellize) {
			for (size_t i = 0; i < vectorSet.size(); i++) {
				UWAssert::UWAssert::shouldNeverHappen(mat_->NumCols() != vectorSet[i]->getLength());
				mat_->addRow(*(vectorSet[i]));
			}
		...
		template <typename T>
		void HowellMatrix<T>::insertSetOfRows(std::vector<ref_ptr<SparseTArray<T> > > & vectorSet, bool perform_howellize) {
			for (size_t i = 0; i < vectorSet.size(); i++) {
				UWAssert::UWAssert::shouldNeverHappen(mat_->NumCols() != vectorSet[i]->GetLength());
				mat_->addRow(vectorSet[i]);
			}
		...
		template <typename T>
		void HowellMatrix<T>::insertSetOfRows(const std::vector<ref_ptr<SparseTArray<T> > > & vectorSet, bool perform_howellize) {
			for (size_t i = 0; i < vectorSet.size(); i++) {
				UWAssert::UWAssert::shouldNeverHappen(mat_->NumCols() != vectorSet[i]->GetLength());
				mat_->addRow(*(vectorSet[i]));
			}
		...
		*/
		...
		void HowellMatrix<T>::reduceToHowellForm() {
			...
			ref_ptr<SparseRowMatrix<T> > howell_form_ks = new SparseRowMatrix<T>(GetVectorSize());
			...
			for (i = 0; i < GetVectorSize(); i++)
			{
				...
				if (r != 0)
				{
					ref_ptr<SparseTArray<T> > log_conseq = new SparseTArray<T>(*(mat_->GetSparseRow(rowToAddToHowellForm)));
				...
				ref_ptr<SparseTArray<T> > rowToAdd = mat_->GetSparseRow(rowToAddToHowellForm);
		...
		template <typename T>
		ref_ptr<GenericHowellMatrix> HowellMatrix<T>::generic_project(const std::vector<size_t> & projectionVariables) const {
			return project(projectionVariables).get_ptr();
		}
		...
		*/
		template <typename T>
		ref_ptr<HowellMatrix<T> > HowellMatrix<T>::project(const std::vector<size_t> & projectionVariables) const
		{
			...
			*/
			...
			for (size_t i = 0; i < reorderedHowellMatrix.size(); i++)
			{
				...
				{
					ref_ptr<SparseTArray<T> > projected_row = new SparseTArray<T>(*(reorderedHowellMatrix.mat_->GetSparseRow(i)));
		...
		template <typename T>
		ref_ptr<HowellMatrix<T> > HowellMatrix<T>::havoc(size_t i) const
		{
			...
			ref_ptr<HowellMatrix<T> > iProjectedOutMatrix = project(projectionVar);

			ref_ptr<HowellMatrix<T> > ret = iProjectedOutMatrix;
		...
		template <typename T>
		ref_ptr<GenericHowellMatrix> HowellMatrix<T>::generic_havoc(std::vector<size_t> havocSet) const {
			return havoc(havocSet).get_ptr();
		}

		template <typename T>
		ref_ptr<HowellMatrix<T> > HowellMatrix<T>::havoc(std::vector<size_t> havocSet) const
		{
			if (havocSet.size() == 0u)
...
#endif
			ref_ptr<HowellMatrix<T> > proj_mat = project(projectionVar);

#ifdef UW_ASSERTS_ENABLED
			UWAssert::UWAssert::shouldNeverHappen(proj_mat->GetVectorSize() != kNumNonHavocVars);
#endif

			ref_ptr<HowellMatrix<T> > ret = proj_mat;
		...
		template <typename T>
		const ref_ptr<ModularRow<T> > HowellMatrix<T>::get(size_t i) const {
#ifdef UW_ASSERTS_ENABLED
			UWAssert::UWAssert::shouldNeverHappen(!(i < mat_->NumRows()));
#endif
			const ref_ptr<ModularRow<T> > ret =
				new ModularRow<T>(mat_->GetRow(i), mat_->NumCols());
			return ret;
		}

		template <typename T>
		const ref_ptr<SparseTArray<T> > HowellMatrix<T>::getSparseRow(size_t i) const {
#ifdef UW_ASSERTS_ENABLED
			UWAssert::UWAssert::shouldNeverHappen(!(i < mat_->NumRows()));
#endif
			return mat_->GetConstSparseRow(i);
		}
		...
		std::vector<std::pair<size_t, BitVector::Value> > HowellMatrix<T>::getRowAsValueCoeffs(size_t r) const {
			const ref_ptr<SparseTArray<T> > row = getSparseRow(r);
			return getRowAsValueCoeffs(row);
		}

		template <typename T>
		std::vector<std::pair<size_t, BitVector::Value> > HowellMatrix<T>::getRowAsValueCoeffs(const ref_ptr<SparseTArray<T> > & sp_arr) const {
		...
		bool HowellMatrix<T>::IsLogicalConsequence(size_t r1, size_t r2) const {
			const ref_ptr<SparseTArray<T> > r1_sp = getSparseRow(r1);
			const ref_ptr<SparseTArray<T> > r2_sp = getSparseRow(r2);
		...
		template <typename T>
		const std::vector<ref_ptr<ModularRow<T> > > HowellMatrix<T>::getRows(size_t i, size_t j) const {
			std::vector<ref_ptr<ModularRow<T> > > ret;
		...
		ModularSquareMatrix<T> HowellMatrix<T>::getSquareMatrix() const
		{
			...
			ref_ptr<TArray<T> > sq_mat = new TArray<T>(GetVectorSize() * GetVectorSize());
			size_t pos = 0;
		...
		*/
		...
		template <typename T>
		void HowellMatrix<T>::setEntryAsValue(size_t rowNum, size_t columnNum, BitVector::Value value)
		{
		...
		*/
		...
		HowellMatrix<T> HowellMatrix<T>::parse(std::istream & buf) {
			...
			ref_ptr<SparseRowMatrix<T> > mat = new SparseRowMatrix<T>(SparseRowMatrix<T>::parse(buf));
