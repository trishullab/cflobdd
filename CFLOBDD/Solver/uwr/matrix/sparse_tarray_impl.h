...
namespace CFL_OBDD
{
	namespace HowellMatrix
	{
		...
		template <typename T>
		ref_ptr<TArray<T> > SparseTArray<T>::GetTArray() const
		{
			ref_ptr<TArray<T> > ret = new TArray<T>(len_);
			typename std::vector<std::pair<size_t, T> >::const_iterator it = arr_.begin();
			for (size_t i = 0; i < len_; i++)
			{
		...
		// So, this increases `len`.
		template <typename T>
		inline void SparseTArray<T>::insertVals(size_t beg_index, size_t length, const SparseTArray<T>& vals, size_t vals_beg_index) {
			if (length == 0)
				return;
