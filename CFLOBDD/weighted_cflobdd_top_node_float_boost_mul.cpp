...
//
template <>
bool WeightedCFLOBDDTopNodeT<BIG_FLOAT, std::multiplies<BIG_FLOAT>>::FindOneSatisfyingAssignment(SH_OBDD::Assignment * &assignment)
{
	for (unsigned int i = 0; i < rootConnection.entryPointHandle->handleContents->numExits; i++) {
		...
		if (k == 1) {  // A satisfying assignment must exist
			unsigned int size = 1 << WeightedCFLOBDDFloatBoostTopNode::maxLevel;
...
namespace CFL_OBDD {
	...
	WeightedCFLOBDDTopNodeFloatBoostRefPtr MkTrueFloatMulTop()
	{
		...
		m.Canonicalize();
		v = new WeightedCFLOBDDFloatBoostTopNode(WeightedCFLOBDDNodeHandleT<BIG_FLOAT, std::multiplies<BIG_FLOAT>>::NoDistinctionNode[CFLOBDD_MAX_LEVEL], m);
		return v;
	}
