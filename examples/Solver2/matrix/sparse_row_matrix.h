...
#include "sparse_tarray.h"
	...
	namespace HowellMatrix
	{
		...
		public:
			...
			SparseRowMatrix(SparseRowMatrix*& m);
			~SparseRowMatrix();
			...
			const TArray<T>* GetRow(size_t r) const;

			const SparseTArray<T>* GetConstSparseRow(size_t r) const;
			SparseTArray<T>* GetSparseRow(size_t r);
			SparseTArray<T>* operator [](size_t j);
		...
		private:
			typedef std::vector<SparseTArray<T>* > RowVector;
			RowVector mat_;
			size_t num_cols_;
		};
	}
