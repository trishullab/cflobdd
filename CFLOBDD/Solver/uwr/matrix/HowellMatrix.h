...
namespace CFL_OBDD
{
	namespace HowellMatrix
	{
		...
		public:
			// Reference-count field needed to support use of ref_ptr<.>
			RefCounter count;
			...
			virtual ref_ptr<GenericHowellMatrix> Copy() const = 0;
			virtual bool isEqual(const GenericHowellMatrix & op2) const = 0;
			virtual bool isEqual(const ref_ptr<GenericHowellMatrix> & op2) const = 0;
			...
			void concatRight(const ref_ptr<GenericHowellMatrix> that) { concatRight(*that); }

			// Concatenate matrices vertically. Produce block matrix:
			// [ this ]
			// [ that ]
			virtual void concatDown(const GenericHowellMatrix & that) = 0;
			void concatDown(const ref_ptr<GenericHowellMatrix> that) { concatDown(*that); }

			// Project away (some) left-hand columns.
			// Really, this is shorthand for project([index,index+1,...,num_vars])
			virtual ref_ptr<GenericHowellMatrix> generic_project_left(size_t index) const = 0;
			virtual ref_ptr<GenericHowellMatrix> generic_project(const std::vector<size_t> & projectionVariables) const = 0;

			virtual ref_ptr<GenericHowellMatrix> generic_havoc(std::vector<size_t> havocSet) const = 0;
		...
		*/
		...
		public:
			...
			bool isEqual(const ref_ptr<HowellMatrix> & op2) const;
			bool isEqual(const GenericHowellMatrix & op2) const;
			bool isEqual(const ref_ptr<GenericHowellMatrix> & op2) const;
			...
			ref_ptr<GenericHowellMatrix> Copy() const
			{
			...
			void insertSetOfRows(const std::vector<ref_ptr<ModularSquareMatrix<T> > > & vectorSet, bool howellize = true);
			void insertSetOfRows(const std::vector<ref_ptr<ModularRow<T> > > & vectorSet, bool howellize = true);
			void insertSetOfRows(const std::vector<ModularRow<T> > & vectorSet, bool howellize = true);
			void insertSetOfRows(const std::vector<ref_ptr<TArray<T> > > & m, bool howellize = true);
			...
			void insertSetOfRows(std::vector<ref_ptr<SparseTArray<T> > > & f, bool howellize = true);
			void insertSetOfRows(const std::vector<ref_ptr<SparseTArray<T> > > & f, bool howellize = true);
			...
			void insertRow(const ref_ptr<TArray<T> > & m, bool perform_howellize = true);
			...
			void insertRow(const ref_ptr<SparseTArray<T> > & f, bool perform_howellize = true);
			...
			// Project away (some) left-hand columns.
			// Really, this is shorthand for project([index,index+1,...,num_vars])
			ref_ptr<HowellMatrix> project_left(size_t index) const;
			ref_ptr<GenericHowellMatrix> generic_project_left(size_t index) const;

			// Concatenate matrices side-by-side. Produce block matrix [this that].
			void concatRight(const GenericHowellMatrix & that);
			void concatRight(const HowellMatrix & that);
			void concatRight(const ref_ptr<HowellMatrix> that);
			// Concatenate matrices vertically. Produce block matrix:
			// [ this ]
			// [ that ]
			void concatDown(const GenericHowellMatrix & that);
			void concatDown(const HowellMatrix & that);
			void concatDown(const ref_ptr<HowellMatrix> that);
			...
			ref_ptr<HowellMatrix> havoc(size_t i) const;
			ref_ptr<HowellMatrix> havoc(std::vector<size_t> havocSet) const;
			ref_ptr<GenericHowellMatrix> generic_havoc(std::vector<size_t> havocSet) const;
			ref_ptr<HowellMatrix> project(const std::vector<size_t> & projectionVariables) const;
			ref_ptr<GenericHowellMatrix> generic_project(const std::vector<size_t> & projectionVariables) const;
			size_t size() const; //Deprecated: Use num_cols instead.
			size_t num_cols() const;
			size_t num_rows() const;
			const ref_ptr<ModularRow<T> > get(size_t) const;
			const ref_ptr<SparseTArray<T> > getSparseRow(size_t) const;

			std::vector<std::pair<size_t, BitVector::Value> > getRowAsValueCoeffs(size_t) const;
			const std::vector<ref_ptr<ModularRow<T> > > getRows(size_t, size_t) const;
		...
		private:
			std::vector<std::pair<size_t, BitVector::Value> > getRowAsValueCoeffs(const ref_ptr<SparseTArray<T> > & sp_arr) const;

			//Data members
			ref_ptr<SparseRowMatrix<T> > mat_;
