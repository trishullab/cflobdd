...
namespace CFL_OBDD
{
	namespace HowellMatrix
	{
		...
		template <typename T>
		bool is_smallest_row(ref_ptr<ModularSquareMatrix<T> > A,
			size_t pos,
			size_t i,
			size_t &j) {
			size_t minpos = j;
		...
		// precondition: pos <= i,j < N
		template <typename T>
		bool is_smallest_col(ref_ptr<ModularSquareMatrix<T> > A,
		...
		//
		template <typename T>
		void find_pivot(ref_ptr<ModularSquareMatrix<T> >A,
			size_t pos,
			size_t &i, //output
			size_t &j  //output
			) {
		...
		template <typename T>
		ModularSquareMatrix<T>::ModularSquareMatrix(const ref_ptr<TArray<T> > & _m, unsigned d)
			: dim(d) {
		...
		template <typename T>
		ref_ptr<TArray<T> > ModularSquareMatrix<T>::getArray() {
			return m;
		}

		template <typename T>
		const ref_ptr<TArray<T> > ModularSquareMatrix<T>::getConstArray() const {
			return m;
		}
		...
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::copy(const ref_ptr<ModularSquareMatrix<T> > & a)
		{
		...
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::transpose(const ref_ptr<ModularSquareMatrix<T> > & mat)
		{
			ref_ptr<ModularSquareMatrix<T> > result = new ModularSquareMatrix<T>(mat->getDim());
		...
		//-------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::add(
			const ref_ptr<ModularSquareMatrix<T> > & mat1,
			const ref_ptr<ModularSquareMatrix<T> > & mat2) {
			UWAssert::UWAssert::shouldNeverHappen(mat1->getDim() != mat2->getDim());

			unsigned N = mat1->getDim();
			ref_ptr<ModularSquareMatrix<T> > result = new ModularSquareMatrix<T>(N);
		...
		//-------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::add_with_check(
			const ref_ptr<ModularSquareMatrix<T> > & mat1,
			const ref_ptr<ModularSquareMatrix<T> > & mat2)
		{
			UWAssert::UWAssert::shouldNeverHappen(mat1->getDim() != mat2->getDim());
		...
		//-------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::combine_with_check(
			const ref_ptr<ModularSquareMatrix<T> > & mat1,
			const ref_ptr<ModularSquareMatrix<T> > & mat2,
			unsigned n) {
			UWAssert::UWAssert::shouldNeverHappen(mat1->getDim() != mat2->getDim());
		...
		//------------------------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::subtract(
			const ref_ptr<ModularSquareMatrix<T> > & mat1,
			const ref_ptr<ModularSquareMatrix<T> > & mat2)
		{
			UWAssert::UWAssert::shouldNeverHappen(mat1->getDim() != mat2->getDim());
		...
		//------------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::multiply(
			const ref_ptr<ModularSquareMatrix<T> > & mat1,
			const ref_ptr<ModularSquareMatrix<T> > & mat2)
		{
			UWAssert::UWAssert::shouldNeverHappen(mat1->getDim() != mat2->getDim());
		...
		// Returns col1 * col2^T
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::createNbyNMatrix(
			unsigned N,
		...
		//------------------------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::mkId(unsigned N) {
			if (N == 0) return NULL;
			ref_ptr<ModularSquareMatrix<T> > result = new ModularSquareMatrix<T>(N);
			for (unsigned i = 0; i < N; i++) {
				for (unsigned j = 0; j < N; j++) {
		...
		//------------------------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::mkPartialId(unsigned N, unsigned start, unsigned stop) {
		...
		//------------------------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::mkSinglePointProjectionMatrix(
		...
		//------------------------------------
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::mkSinglePointMatrix(
		...
		*/
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::inflateTo2K(const ref_ptr<ModularSquareMatrix<T> > & mat)
		{
			unsigned N = mat->getDim();
			unsigned K = N - 1;
			unsigned inflatedN = K * 2 + 1; // N + K

			ref_ptr<ModularSquareMatrix<T> > result = new ModularSquareMatrix<T>(inflatedN);
			// copy from (0,0) ~ (0,N-1)
			for (unsigned i = 0; i < N; i++) {
		...
		template <typename T>
		void ModularSquareMatrix<T>::modular_svd(ref_ptr<ModularSquareMatrix<T> > & A,
			ref_ptr<ModularSquareMatrix<T> > & Rinv,
			ref_ptr<ModularSquareMatrix<T> > & Linv) {
			...
			ref_ptr<ModularSquareMatrix<T> > original_A = ModularSquareMatrix<T>::copy(A);
		...
		//
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::replace_column(
			const ref_ptr<ModularSquareMatrix<T> > & mat, unsigned index, const std::vector<T> & col
			) {
			UWAssert::UWAssert::shouldNeverHappen(index >= (int)col.size());

			unsigned N = mat->getDim();

			ref_ptr<TArray<T> > m = new TArray<T>(mat->getConstMatrix(), N*N);
			ref_ptr<ModularSquareMatrix<T> > result = new ModularSquareMatrix<T>(m, N);
		...
		//
		template <typename T>
		std::vector<T> ModularSquareMatrix<T>::get_column_of_index(const ref_ptr<ModularSquareMatrix<T> > & mat, int index)
		{
			std::vector<T> ans;
		...
		*/
		template <typename T>
		ref_ptr<ModularSquareMatrix<T> > ModularSquareMatrix<T>::dualize(const ModularSquareMatrix<T> & M)
		{
			ref_ptr<ModularSquareMatrix<T> > Linv = new ModularSquareMatrix<T>(M.getDim());
			ref_ptr<ModularSquareMatrix<T> > Rinv = new ModularSquareMatrix<T>(M.getDim());
			ref_ptr<ModularSquareMatrix<T> > D = new ModularSquareMatrix<T>(M);

			//Obtain D, Rinv, Linv by using modular_svd method.
			modular_svd(D, Rinv, Linv);

			ref_ptr<ModularSquareMatrix<T> > Rinv_transpose = transpose(Rinv);
			ref_ptr<ModularSquareMatrix<T> > Linv_transpose = transpose(Linv);

			//Obtain T from D
			for (size_t i = 0; i < D->getDim(); i++) {
				D->set(i, i, BitVector::mult_pow2<T>(1, BitVector::highest_power<T>() - BitVector::compute_rank(D->at(i, i))));
			}
			return multiply(Linv_transpose, multiply(D, Rinv_transpose));
		}
