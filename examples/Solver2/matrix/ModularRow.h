...
namespace HowellMatrix
{
	...
	public:
		...
		TArray(TArray* ia);  // Copy constructor.
		TArray(T* ia, size_t length); // Copies the contents of memory at ia.
		...
		TArray<T>* projectOut(const std::vector<size_t> & projectOutIndices) const;
	...
	*/
	...
	public:
		...
		ModularRow(const TArray<T>* & _m, size_t n);
		ModularRow(const TArray<T>* & _m, size_t _li, size_t _lr);
		...
		TArray<T>* getTArray();
		TArray<T>* getConstTArray() const;

		void setMat(TArray<T>* mat);
		...
		static ModularRow* copy(const ModularRow* a);
